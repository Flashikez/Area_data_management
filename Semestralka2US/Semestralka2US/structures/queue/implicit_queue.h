...
namespace structures
{
	...
	inline ImplicitQueue<T>& ImplicitQueue<T>::operator=(const ImplicitQueue<T>& other)
	{
		...
		if (this != &other)
		{
			...
			{
				throw std::logic_error("Queue capacity missmatch!");
			}
			if (other.startIndex_ + other.size_ > other.array_->size())
			{
				int remaining = other.array_->size() - other.size_;
				Array<T>::copy(*other.array_, other.startIndex_, *array_, 0, remaining);
				Array<T>::copy(*other.array_, 0, *array_, remaining, other.size_ - remaining);
			}
			else
			{
				Array<T>::copy(*other.array_, other.startIndex_, *array_, 0, size_);
			}
			startIndex_ = 0;
			size_ = other.size_;
		}
	...
	inline T ImplicitQueue<T>::pop()
	{
		...
		size_--;
		// ak je prazdy... vynimka
		// ak vytiahnem prvok zo sizeindexu
		return pomPrvok;
	}
