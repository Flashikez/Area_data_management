...
namespace structures
{
	...
	inline HashTable<K, T>& HashTable<K, T>::operator=(const HashTable<K, T>& other)
	{
		...
		if (this != &other)
		{
			if (data_->size() != other.data_->size()) {
				throw std::invalid_argument("HASHTABLE// Arrays' sizes differ!");
			}
			else {
				this->clear();
				for (int i = 0; i < other.DATA_ARRAY_SIZE; i++)
				{
					if ((*data_)[i] == nullptr && (*other.data_)[i] != nullptr) {
						(*data_)[i] = new UnsortedSequenceTable <K, T>();
					}
	...
	inline T & HashTable<K, T>::operator[](const K key)
	{
		...
		if ((*data_)[hash] == nullptr) {
			throw std::logic_error("HASHTABLE// Tabulka neobsahuje prvok s danym klucom");
		}
		else {
			try {
				Table <K, T>* tab = (*data_)[hash];
				return ((*tab)[key]);
			}
	...
	inline void HashTable<K, T>::insert(const K & key, const T & data)
	{
		...
		if ((*data_)[hash] == nullptr) {
			(*data_)[hash] = new UnsortedSequenceTable<K, T>();
		}
		try {

		(*data_)[hash]->insert(key, data);
		size_++;
		}
		catch (std::logic_error) {
			throw std::logic_error("HASHTABLE// Tabulka uz obsahuje prvok s danym klucom");
		}
	...
	inline T HashTable<K, T>::remove(const K & key)
	{
		...
		if ((*data_)[hash] == nullptr) {
			...
			T data = (*data_)[hash]->remove(key);
			size_--;
			if ((*data_)[hash]->size() == 0) {
				delete (*data_)[hash];
	...
	inline bool HashTable<K, T>::tryFind(const K & key, T & data)
	{
		...
		if ((*data_)[hash] == nullptr) {
			...
			return (*data_)[hash]->tryFind(key, data);
	...
	inline bool HashTable<K, T>::containsKey(const K & key)
	{
		...
		if ((*data_)[hash] == nullptr) {
			...
			return (*data_)[hash]->containsKey(key);
		}
	...
	inline void HashTable<K, T>::clear()
	{
		//TODO 09: HashTable
		for (int i = 0; i < DATA_ARRAY_SIZE; i++) {
			if ((*data_)[i] != nullptr) { // ak tabulka uz existuje
				(*data_)[i]->clear();
				delete (*data_)[i];
				(*data_)[i] = nullptr;
	...
	inline int HashTable<K, T>::hashFunction(const K & key) const
	{
		...
		for (int i = 0; i < key.length(); i++)
		{
			...
			return hash % DATA_ARRAY_SIZE;
		


		//return key % DATA_ARRAY_SIZE;
	}
	...
	inline Iterator<TableItem<K, T>*>& HashTable<K, T>::HashTableIterator::operator=(const Iterator<TableItem<K, T>*>& other)
	{
		...
		iterLast_ = dynamic_cast<const HashTableIterator&>(other).iterLast_;
	...
	inline bool HashTable<K, T>::HashTableIterator::operator!=(const Iterator<TableItem<K, T>*>& other)
	{
		...
		return position_ != dynamic_cast<const HashTableIterator&>(other).position_;
	...
	inline void HashTable<K, T>::HashTableIterator::movePositionInArray()
	{
		...
		int pocIndex = position_;

		for (int i = position_+1; i < DATA_ARRAY_SIZE; i++)
		{
			if ((*(*hashTable_).data_)[i] != nullptr) {
				position_ = i;
				break;
			}

		}
		if (position_ == pocIndex) {  // ked sa nenasla ziadna dalsia tabulka
			position_ = DATA_ARRAY_SIZE;
			return;
		}

		if (position_ < DATA_ARRAY_SIZE) {
			Table <K, T>* tab = (*(*hashTable_).data_)[position_];
			if (tab != nullptr) {
				iterCurr_ = tab->getBeginIterator();
			
			iterLast_ = tab->getEndIterator();
			}
