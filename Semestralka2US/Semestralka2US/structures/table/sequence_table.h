...
namespace structures 
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		...
		{
			clear();
			for each (auto var in other)
			{
				list_->add(new TableItem<K, T>(var->getKey(), var->accessData()));
			}
		}
		return *this;
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (item == nullptr)
			throw std::logic_error("Tabulka neobsahuj prvok s tymto klucom");
	...
	inline const T SequenceTable<K, T>::operator[](const K key) const
	{
		...
		TableItem<K, T>* item = findTableItem(key);
		if (item == nullptr)
			throw std::logic_error("Tabulka neobsahuj s tymto klucom");
	...
	{
		//TODO 09: SequenceTable
		if(containsKey(key) != false) throw std::logic_error("Tabulka uz obsahuje prvok s danym klucom");
		else {
	...
	inline T SequenceTable<K, T>::remove(const K & key)
	{
		...
		TableItem<K, T>* item = findTableItem(key);
		if (item == nullptr)
			throw std::logic_error("Tabulka neobsahuj prvok na danom indexu");
