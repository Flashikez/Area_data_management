...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		...
		if (isRoot())
		{
			throw new std::logic_error("Root has no brother!");
		}
		else
		{
	...
	inline size_t TreeNode<T>::sizeOfSubtree()
	{
		...
		for (int i = 0; i < degree(); i++)
		{
			...
			//result += son != nullptr ? son->sizeOfSubtree() : 0;  //Nahrada za cely if
			if (getSon(i) != nullptr)
			{
	...
	{
		//TODO 08: Tree<T>::TreeIterator
		const TreeIterator& iter = dynamic_cast<const Tree<T>::TreeIterator&>(other);
		*path_ = *iter.path_;
		return *this;
	}
