...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		list_->add(newItem);
		int indexItem = size() - 1;
		int indexParent = getParentIndex(indexItem);
		while (indexItem > 0 && (*list_)[indexParent]->getPriority() > (*list_)[indexItem]->getPriority())
		{
			DSRoutines::swap((*list_)[indexParent], (*list_)[indexItem]);
			indexItem = indexParent;
			indexParent = getParentIndex(indexItem);
		}
	}
	...
	{
		//TODO 06: Heap
		// unsorted mozes urobit linked list + arrayUnsorted
		// dvojzoznam za 2 body
